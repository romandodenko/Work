...
const arrowTexture = new PIXI.Texture.from('images/arrow.png');
  ...
  const centerImg = new PIXI.Texture.from('images/spqr.png');
  const centerChildImg = new PIXI.Sprite(centerImg);
  centerChildImg.x = app.screen.width / 2 - 49;
  centerChildImg.y = app.screen.height/ 2 - 49;
  centerChildImg.width = 100;
  centerChildImg.height = 100;
...
for (let sector = 0; sector < numberOfSectors; sector += 1) {
  ...
  sectionGraphic.beginFill(`0xCD5C5C`);
