...
document.querySelector('.current-date').textContent = `${now.toLocaleDateString('es', {month: "long", day: "2-digit", year: "numeric",  hour: "2-digit", minute: "2-digit"})}`;
...
document.querySelector('.data-end-scr').textContent = `${now.toLocaleDateString('es', {month: "long", day: "2-digit", year: "numeric"})}`;
...
inputValue.addEventListener('input', function(e) {
    ...
    profit.textContent = `${this.value}` * 825;

    if(this.value == 0) {
        profit.textContent = 825;
        days.textContent = 1;
    }
})